...
exports.deleteBook = (req, res) => {
  ...
  allListDB.updateOne(
    ...
    .then((data) => {
      if (!data) {
        res.status(404).send({
          message: `Cannot delete book. Maybe book was not found!`,
        });
      } else {
        res.send({
...
exports.addBookToAList = (req, res) => {
  const uid = req.params.uid;
  const titrelist = req.params.titrelist;
...
exports.deleteBookFromList = (req, res) => {
  const uid = req.params.uid;
  const titrelist = req.params.titrelist;
  const isbn = req.params.isbn;
...
// Delete a Book from All Lists
exports.deleteBookFromAllLists = (req, res) => {
  const uid = req.params.uid;
  const isbn = req.params.isbn;
  ...
  allListDB.collection
    ...
    .then((data) => {
      if (!data) {
        ...
        res.send({
          message: "Book was deleted successfully from all lists!",
        });
      }
    })
    .catch((err) => {
...
// Find a Book from One List
exports.findBookFromOneList = (req, res) => {
  const uid = req.params.uid;
  const titrelist = req.params.titrelist;
  const isbn = req.params.isbn;

  allListDB.collection
    .findOne({ uid: uid, titrelist: titrelist, "books.isbn": isbn })
    .then((data) => {
      if (!data) {
        res.status(404).send({
