...
export class partagerListe implements OnInit {
  ...
  async partagerListe() {
    ...
    if (this.data.email == undefined) {
      ...
      let list = await this.film.getOneListFilmAsync(list_title);
      for (let i = 0; i < list.movies.length; i++) {
        list.movies[i].accompagnateurs = '';
        list.movies[i].dateVision = '';
        list.movies[i].cinema = '';
      }
      let list_destinaire = await this.film.getOneListFilmAsyncDestinaire(
        list_title,
        dest[0]._id
      );
      if (list_destinaire) {
        this.film.deleteListOfAllLists(list_destinaire._id);
        await this.film.shareListFilmAsync(dest[0]._id, list);
        this.onNoClick();
        this.openSnackBar(
