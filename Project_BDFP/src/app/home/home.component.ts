...
import { ajouterBook } from './home.component-dialog.book';
...
export class HomeComponent implements OnInit {
  ...
  currentIcon: string = 'local_movies'; // Initial icon
  ...
  ngOnInit(): void {
    ...
    //Valeur menu deroulant
    this.dataService.selectedCategory$.subscribe((category) => {
      this.selectedCategory = category;
      // Update the number of films when the selected category changes
      this.updateNumberOfFilms();

      // Mettez à jour l'icône en fonction de la catégorie sélectionnée
      this.currentIcon = category === 'film' ? 'local_movies' : 'book';
    });
  }
  ...
  openDialogBook(): void {
    const dialogRef = this.diag.open(ajouterBook, {
      width: '800px',
      data: {
...
export class ajouterFilm implements OnInit {
  ...
  async getAccompagnateursHistory() {
    let accompagnateursHistory =
      await this.filmService.getAccompagnateursHistory();
  ...
  async deleteAccompagnateurHistory(event: MouseEvent, accompagnateur: string) {
    event.stopPropagation();
    let accompagnateursHistory =
      await this.filmService.deleteAccompagnateursHistory(accompagnateur);
  ...
  async ajoutFilmFromOMDB() {
    ...
    if (
      ...
      if (
        this.data.accompagnateurs != null &&
        this.data.accompagnateurs != undefined &&
        this.data.accompagnateurs != ''
      ) {
      ...
      if (
        this.data.cinema != null &&
        this.data.cinema != undefined &&
        this.data.cinema != ''
      ) {
  ...
  async ajoutFilmFromTMDB() {
    ...
    this.api.getMovieTMDbId(tmdbid).subscribe(async (movieTMDB: any) => {
      ...
      if (
        ...
        let title =
          this.data.french_title != undefined &&
          this.data.french_title.trim() != ''
            ? this.data.french_title
            : this.selectedMovie.title;
        console.log(title);
